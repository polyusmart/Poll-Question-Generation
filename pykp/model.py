...
class Seq2SeqModel(nn.Module):
    ...
    def __init__(self, opt):
        ...
        #     topic_num=self.topic_num
        # )
        # self.decoder_2 =copy.deepcopy(self.decoder)
        ...
        if self.share_embeddings:
            ...
            self.decoder_2.embedding.weight = self.decoder.embedding.weight
    ...
    def forward(self, src, src_lens, trg, src_oov, max_num_oov, src_mask, topic_represent,tag, num_trgs=None):
        """
        ...
        max_tag_length = tag.size(1)

        decoder_dist_all = []
        attention_dist_all = []
        decoder_dist_all_tag = []
        attention_dist_all_tag = []
        ...
        y_t_init_tag = tag.new_ones(batch_size) * self.bos_idx  # [batch_size]
        ...
        for t in range(max_tag_length):
            if t == 0:
                h_t_tag = h_t_init
                y_t_tag = y_t_init_tag
            else:
                h_t_tag = h_t_next_tag
                y_t_tag = y_t_next_tag

            decoder_dist_tag, h_t_next_tag, _, attn_dist_tag, p_gen, coverage = self.decoder_2(y_t_tag, topic_represent, h_t_tag, memory_bank, src_mask, max_num_oov, src_oov, coverage)
            decoder_dist_all_tag.append(decoder_dist_tag.unsqueeze(1))  # [batch, 1, vocab_size]
            attention_dist_all_tag.append(attn_dist_tag.unsqueeze(1))  # [batch, 1, src_seq_len]
            y_t_next_tag = tag[:, t]  # [batch]

        decoder_dist_all_tag = torch.cat(decoder_dist_all_tag, dim=1)  # [batch_size, trg_len, vocab_size]
        attention_dist_all_tag = torch.cat(attention_dist_all_tag, dim=1)  # [batch_size, trg_len, src_len]
        ...
        return decoder_dist_all, h_t_next, attention_dist_all, encoder_final_state, coverage_all, None, None, None, decoder_dist_all_tag, attention_dist_all_tag, h_t_next_tag
