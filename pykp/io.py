...
class KeyphraseDataset(torch.utils.data.Dataset):
    def __init__(self, examples, word2idx, idx2word, bow_dictionary,
        ...
        keys = ['src', 'trg', 'trg_copy', 'src_oov', 'oov_dict', 'oov_list', 'src_str', 'trg_str', 'src_bow','trg_tag', 'trg_tag_copy','trg_tag_str']
    ...
    def collate_fn_one2one(self, batches):
        ...
        tag = [b['trg_tag'] + [self.word2idx[EOS_WORD]] for b in batches]
        ...
        tag_oov = [b['trg_tag_copy'] + [self.word2idx[EOS_WORD]] for b in batches]
        ...
        seq_pairs = sorted(zip(src, trg, trg_oov, src_oov, oov_lists, src_bow,tag,tag_oov), key=lambda p: len(p[0]), reverse=True)
        src, trg, trg_oov, src_oov, oov_lists, src_bow,tag,tag_oov = zip(*seq_pairs)
        ...
        tag, tag_lens, tag_mask = self._pad(tag)
        # trg_target, _, _ = self._pad(trg_target)
        trg_oov, _, _ = self._pad(trg_oov)
        tag_oov, _, _ = self._pad(tag_oov)
        src_oov, _, _ = self._pad(src_oov)
        src_bow = self._pad_bow(src_bow)

        return src, src_lens, src_mask, trg, trg_lens, trg_mask, src_oov, trg_oov, oov_lists, src_bow, tag, tag_lens, tag_mask,tag_oov
    ...
    def collate_fn_one2many(self, batches):
        ...
        tag_str = [b['trg_tag_str'] for b in batches]
        ...
        if self.load_train:
            ...
            seq_pairs = sorted(zip(src, src_oov, oov_lists, src_str, trg_str, original_indices, src_bow, tag_str),
                               key=lambda p: len(p[0]), reverse=True)
            src, src_oov, oov_lists, src_str, trg_str, original_indices, src_bow, tag_str = zip(*seq_pairs)
        ...
        if self.load_train:
            ...
            tag_lens, tag_mask = None, None


        src_bow = self._pad_bow(src_bow)

        return src, src_lens, src_mask, src_oov, oov_lists, src_str, trg_str, trg, trg_oov, trg_lens, trg_mask, original_indices, src_bow,tag_str, tag, tag_oov, tag_lens, tag_mask
...
def build_dataset(src_trgs_pairs, word2idx, bow_dictionary, opt, mode='one2one', include_original=True):
    ...
    for idx, (source, targets, conv, tags) in enumerate(src_trgs_pairs):
        ...
        for target in targets:
            ...
            L=[len(t) for t in tags]
            hashtag=tags[np.argmax(L)]
            ...
            if include_original:
                ...
                example['trg_tag_str'] = hashtag

            example['src'] = src

            if opt.bow_data_soure=="post":
                example['src_bow'] = bow_dictionary.doc2bow(source)
            else:
                example['src_bow'] = bow_dictionary.doc2bow(conv)

            if len(bow_dictionary.doc2bow(source)) == 0  or len(bow_dictionary.doc2bow(conv)) == 0:
                zero_bow += 1
            ...
            tag = [word2idx[w] if w in word2idx and word2idx[w] < opt.vocab_size
                   else word2idx[UNK_WORD] for w in hashtag]
            ...
            for w in hashtag:
                if w in word2idx and word2idx[w] < opt.vocab_size:
                    tag_copy.append(word2idx[w])
                elif w in oov_dict:
                    tag_copy.append(oov_dict[w])
                else:
                    tag_copy.append(word2idx[UNK_WORD])
            example['trg_tag_copy'] = tag_copy
