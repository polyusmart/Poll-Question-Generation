    ...
    for epoch in range(opt.start_epoch, opt.epochs + 1):
        if Train_Seq2seq:
            ...
            for batch_i, batch in enumerate(train_data_loader):
                total_batch += 1
                batch_loss_stat, _ = train_one_batch(batch, model, ntm_model, optimizer, opt, batch_i, epoch)
            ...
            if not opt.save_each_epoch and num_stop_dropping >= opt.early_stop_tolerance and epoch>3:  # not opt.joint_train or
...
def train_one_batch(batch, model, ntm_model, optimizer, opt, batch_i, epoch):
    # train for one batch
    src, src_lens, src_mask, trg, trg_lens, trg_mask, src_oov, trg_oov, oov_lists, src_bow, tag, tag_lens, tag_mask,tag_oov = batch
    ...
    tag_mask = tag_mask.to(opt.device)
    src_oov = src_oov.to(opt.device)
    trg_oov = trg_oov.to(opt.device)
    tag_oov = tag_oov.to(opt.device)
    ...
    # for one2one setting
    decoder_dist, h_t, attention_dist, encoder_final_state, coverage, _, _, _, decoder_dist_tag,attention_dist_tag, h_t_tag \
        = model(src, src_lens, trg, src_oov, max_num_oov, src_mask, topic_represent, tag)
    ...
    if opt.copy_attention:  # Compute the loss using target with oov words
        ...
        loss_tag = masked_cross_entropy(decoder_dist_tag, tag_oov, tag_mask, tag_lens,
                                    opt.coverage_attn, coverage, attention_dist_tag, opt.lambda_coverage,
                                    opt.coverage_loss)
        ...
        loss_tag = masked_cross_entropy(decoder_dist_tag, tag, tag_mask, tag_lens,
                                    opt.coverage_attn, coverage, attention_dist_tag, opt.lambda_coverage, opt.coverage_loss)
    ...
    # loss_tag.div(normalization).backward()

    loss.div(normalization)
    loss_tag.div(normalization)


    # ===============================
    totall_loss=loss+loss_tag
    # ===============================
    # if epoch <=2:
    #     totall_loss=loss_tag
    # if epoch ==3:
    #     totall_loss=loss+loss_tag
    # if epoch >3:
    #     totall_loss=loss
    ...
    stat = LossStatistics(totall_loss.item(), total_trg_tokens+total_tag_tokens , n_batch=1, forward_time=forward_time,
