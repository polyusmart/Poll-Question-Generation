...
def read_src_trg_files(opt, tag="train"):
    ...
    if tag == "train":
        ...
        conv_file=opt.train_conv
        tag_file = opt.train_tag
    elif tag == "valid":
        src_file = opt.valid_src
        trg_file = opt.valid_trg
        conv_file = opt.valid_conv
        tag_file = opt.valid_tag
    else:
        src_file = opt.test_src
        trg_file = opt.test_trg
        conv_file = opt.test_conv
        tag_file = opt.test_tag

    tokenized_src = []
    tokenized_trg = []
    tokenized_conv = []
    tokenized_tag = []
    for src_line, trg_line, conv_line,tag_ine in zip(open(src_file,'r',encoding='utf-8'), open(trg_file,'r',encoding='utf-8'),open(conv_file,'r',encoding='utf-8'),open(tag_file,'r',encoding='utf-8')):
        # process src and trg line
        src_word_list = src_line.strip().split(' ')
        conv_word_list = conv_line.strip().split(' ')
        trg_list = trg_line.strip().split(';')  # a list of target sequences
        trg_word_list = [trg.strip().split(' ') for trg in trg_list]

        tag_list = tag_ine.strip().split(';')  # a list of target sequences
        tag_word_list = [tag.strip().split(' ') for tag in tag_list]

        # Truncate the sequence if it is too long
        src_word_list = src_word_list[:opt.max_src_len]
        conv_word_list= conv_word_list[:opt.max_src_len]
        if tag != "test":
        ...
        tokenized_conv.append(conv_word_list)
        tokenized_tag.append(tag_word_list)

    assert len(tokenized_src) == len(tokenized_trg)==len(tokenized_conv)==len(tokenized_tag), \
        'the number of records in source and target are not the same'

    tokenized_pairs = list(zip(tokenized_src, tokenized_trg,tokenized_conv,tokenized_tag))
...
def build_vocab(tokenized_src_trg_pairs):
    ...
    for src_word_list, trg_word_lists, conv_word_list,tag_word_list in tokenized_src_trg_pairs:
...
def make_bow_dictionary(tokenized_src_trg_pairs, data_dir, bow_vocab):
    ...
    for src, tgt, conv,tag in tokenized_src_trg_pairs:
        cur_bow = []
        cur_bow.extend(src)
        for t in tgt:
            cur_bow.extend(t)
...
if __name__ == "__main__":
    ...
    opt.train_conv = opt.data_dir + '/train_conv.txt'
    opt.valid_src = opt.data_dir + '/valid_src.txt'
    opt.valid_trg = opt.data_dir + '/valid_trg.txt'
    opt.valid_conv = opt.data_dir + '/valid_conv.txt'
    opt.test_src = opt.data_dir + '/test_src.txt'
    opt.test_trg = opt.data_dir + '/test_trg.txt'
    opt.test_conv = opt.data_dir + '/test_conv.txt'

    opt.train_tag = opt.data_dir + '/train_choice.txt'
    opt.valid_tag = opt.data_dir + '/valid_choice.txt'
    opt.test_tag = opt.data_dir + '/test_choice.txt'
