...
def evaluate_loss(data_loader, model, ntm_model, opt):
    ...
    with torch.no_grad():
        for batch_i, batch in enumerate(data_loader):
            ...
            # evaluation_loss_sum += loss_tag.item()
            evaluation_loss_sum += loss.item() + loss_tag.item()
            total_trg_tokens += sum(trg_lens)
            total_tag_tokens += sum(tag_lens)

            if (batch_i + 1) % (len(data_loader) // 5) == 0:
                print("Train: %d/%d batches, current avg loss: %.3f" %
                      ((batch_i + 1), len(data_loader), evaluation_loss_sum / (total_trg_tokens+ total_tag_tokens)             ))

    # eval_loss_stat = LossStatistics(evaluation_loss_sum, total_tag_tokens, n_batch, forward_time=forward_time_total,
    #                                 loss_compute_time=loss_compute_time_total)

    eval_loss_stat = LossStatistics(evaluation_loss_sum, total_trg_tokens+ total_tag_tokens, n_batch, forward_time=forward_time_total,
                                    loss_compute_time=loss_compute_time_total)
    ...
    with torch.no_grad():
        ...
        for batch_i, batch in enumerate(one2many_data_loader):
            if (batch_i + 1) % interval == 0:
                ...
                sys.stdout.flush()
            src, src_lens, src_mask, src_oov, oov_lists, src_str_list, trg_str_2dlist, _, _, _, _, original_idx_list, src_bow, tag_str_2dlist, _, _, _, _  = batch
            """
